...
if (isset($_POST["Login-submit"])){
	...
	if(!mysqli_stmt_prepare($stmt,$sql)){
			header("Location: placements.php?error=sqlerror");
		    exit();
	}
	else{
		...
		if($row=$result->fetch_assoc()){
            ...
            if(password_verify($pwd, $row["password"])){
            	...
            	header("Location: placements.php?login=success");
		        exit();
		        ...
		        header("Location: placements.php?error=wrongpassword");
		         exit();
            }
		}
		else{
		echo"<script>alert('no user')</script>";
			header("Location: placements.php?error=nouser");
		 exit();
		}
	}
